...
import { normalizeRecoveryCodeForVerification } from './mfa-totp.util';
...
export class TwoFactorService {
  ...
  private async consume(tx: Prisma.TransactionClient, userId: string, code: string): Promise<void> {
    ...
    const data = await this.verifyCode(user, code);
  ...
  private async verifyCode(user: User, code: string): Promise<Prisma.UserUpdateManyMutationInput> {
    ...
    let index = this.totp.verifyRecoveryCode(code, hashes);
    index ??= await this.legacyRecoveryIndex(code, hashes);
    if (index === null)
  ...
  private async legacyRecoveryIndex(code: string, hashes: string[]): Promise<number | null> {
    const normalized = normalizeRecoveryCodeForVerification(code);
    for (let index = 0; index < hashes.length; index++) {
      const hash = hashes[index];
      if (hash?.startsWith('$argon2') && (await argon2.verify(hash, normalized))) return index;
    }
    return null;
  }
