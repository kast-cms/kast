...
describe('Trust and access (e2e)', () => {
  ...
  it('enforces MFA for passwords and OAuth, consumes codes once, and revokes access immediately', async () => {
    ...
    const primary = async (): Promise<request.Response> =>
      request(server)
        ...
        .set('User-Agent', 'Trust test browser')
        .send({ email, password })
        .expect(200);
    ...
    const oauth = await app.get(AuthService).oauthCallback('oidc:https://issuer.example', {
      id: 'subject',
      provider: 'oidc',
    ...
    expect(secondDevices.find((device: { current: boolean }) => device.current).userAgent).toBe(
      'Trust test browser',
    );
  ...
  it('preserves existing MFA enrollments and Argon2 recovery codes through migration', async () => {
    const secret = 'JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP';
    const hash = await argon2.hash('ABCD1234ABCD');
    await prisma.user.update({
      where: { id: userId },
      data: {
        twoFactorEnabled: false,
        mfaSecret: app.get(SecretEncryptionService).encrypt(secret),
        mfaEnabledAt: new Date(),
        mfaRecoveryCodes: [hash],
      },
    });
    const migration = readFileSync(
      join(__dirname, '../prisma/migrations/20260915000000_trust_access/migration.sql'),
      'utf8',
    );
    ...
    const pending = await request(server)
      ...
      .send({ email, password })
      .expect(200);
    expect(pending.body.data.mfaRequired).toBe(true);
    const signedIn = await request(server)
      .post('/api/v1/auth/mfa/challenge')
      .send({ challengeToken: pending.body.data.challengeToken, code: 'ABCD-1234-ABCD' })
      .expect(200);
    const status = await request(server)
      .get('/api/v1/auth/mfa')
      .set(...bearer(signedIn.body.data.accessToken))
      .expect(200);
    expect(status.body.data).toMatchObject({ enabled: true, recoveryCodeCount: 0 });
    await request(server)
