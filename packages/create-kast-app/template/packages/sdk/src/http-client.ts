...
/**
 * A failed API call. `code` is the API's machine-readable reason (e.g.
 * `SEO_VALIDATION_WARNINGS`) and is what callers should branch on — the
 * `message` is prose and may be localised or reworded. `details` carries the
 * specifics behind the failure when the API supplied any.
 */
export type KastApiError = Error & {
  code?: string;
  status?: number;
  details?: unknown;
};
...
export class KastHttpClient {
  ...
  private buildError(json: unknown, status: number): KastApiError {
    const err = (json as { error?: { message?: string; code?: string; details?: unknown } }).error;
    const message = err?.message ?? `HTTP ${status}`;
    const error = new Error(message) as KastApiError;
    if (err?.code !== undefined) error.code = err.code;
    // Machine-readable specifics the API attached to the failure — e.g. the list
    // of SEO issues that blocked a publish, or the field errors from validation.
    // Without these a caller can report that something failed but not what.
